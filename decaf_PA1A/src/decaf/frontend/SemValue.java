 ...
 * 
 * 节点的语义值
 * int code - 由yyac分配的代表每一种终结符的整数值
 * Location loc - 文法符号在代码中的位置
 * int typeTag - 常量的种类标识
 * Object literal - 常量的值
 * String ident - 标识符的名字
 * List<ClassDef> clist - 类定义列表
 * List<Tree> flist - 域列表
 * List<VarDef>vlist - 变量定义列表
 * List<Tree> slist - 语句列表
 * List<Expr> elist - 表达式列表
 * TopLevel prog - 顶层树节点，表示整个程序
 * ClassDef cdef - 类定义
 * VarDef vdef - 变量定义
 * MethodDef fdef - 方法定义
 * TypeLiteral type - 各种type的抽象基类
 * Tree stmt - 对应的语句
 * Expr expr - 对应的表达式
 * LValue lvalue - 对应的左值
 */
