...
public class Parser extends Table {
    ...
    private SemValue parse(int symbol, Set<Integer> follow) {
        ...
        Set<Integer> end = new HashSet<>(follow); // end(symbol) for right-hand non-terminal and error recovery
        ...
        if (result != null) {
        	...
        	error();
        	Set<Integer> b = beginSet(symbol);  
        	while (!(b.contains(lookahead) || end.contains(lookahead))) {
        		lookahead = lex();
        	}        	
        	if (b.contains(lookahead)) {
